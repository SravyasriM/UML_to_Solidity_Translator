 ...
 */
	...
	{
			int32 intBits = int32(Float.floatToRawIntBits(a));
 ...
 */
	...
	{
			int64 longBits = int64(Double.doubleToRawLongBits(a));
 ...
 */
	...
	{
			int64 r = int64(x * y);
			int64 ax = int64(Math.abs(x));
			int64 ay = int64(Math.abs(y));
			if(((ax | ay) >>> 31 != 0))
			{
 ...
 */
	...
	{
			int32 r = int32(x - floorDiv(x, y) * y);
			return r;
	}
 ...
 */
	...
	int64 static  negativeZeroFloatBits = int64(Float.floatToRawIntBits(-0;)

	int64 static  negativeZeroDoubleBits = int64(Double.doubleToRawLongBits(-0;)
 ...
 */
	...
	function max(int32 a, int32 b) public returns(int32)
	{
			if(a != a)
			{
				...
				return a;			}
			if((a == 0) && (b == 0) && (Float.floatToRawIntBits(a) == negativeZeroFloatBits))
			{
 ...
 */
	...
	function max(int64 a, int64 b) public returns(int64)
	{
			if(a != a)
			{
				...
				return a;			}
			if((a == 0) && (b == 0) && (Double.doubleToRawLongBits(a) == negativeZeroDoubleBits))
			{
					// Raw conversion ok since NaN can't map to -0.0.
 ...
 */
	...
	function min(int32 a, int32 b) public returns(int32)
	{
			if(a != a)
			{
				...
				return a;			}
			if((a == 0) && (b == 0) && (Float.floatToRawIntBits(b) == negativeZeroFloatBits))
			{
					// Raw conversion ok since NaN can't map to -0.0.
 ...
 */
	...
	function min(int64 a, int64 b) public returns(int64)
	{
			if(a != a)
			{
				...
				return a;			}
			if((a == 0) && (b == 0) && (Double.doubleToRawLongBits(b) == negativeZeroDoubleBits))
			{
					// Raw conversion ok since NaN can't map to -0.0.
 ...
 */
	...
	{
			return (d == 0 || Double.isNaN(d)) ? d : copySign(1, d);
	}
 ...
 */
	...
	{
			return (f == 0 || Float.isNaN(f)) ? f : copySign(1, f);
	}
 ...
 */
	...
	function getExponent(int64 d) public returns(int32)
	{
         ...
         */
			...
			return int32((((Double.doubleToRawLongBits(d) & DoubleConsts.EXP_BIT_MASK) >> (DoubleConsts.SIGNIFICAND_WIDTH - 1)) - DoubleConsts.EXP_BIAS));
	}
	...
	function nextAfter(int64 start, int64 direction) public returns(int64)
	{
			...
			else
			{
				...
				else
				{
						...
						int64 transducer = int64(Double.doubleToRawLongBits(start + 0));
						/*
	...
	function nextAfter(int32 start, int64 direction) public returns(int32)
	{
			...
			else
			{
				...
				else
				{
						...
						int32 transducer = int32(Float.floatToRawIntBits(start + 0));
						/*
             * IEEE 754 floating-point numbers are lexicographically
             * ordered if treated as signed- magnitude integers .
             * Since Java's integers are two's complement,
 ...
 */
	...
	function nextUp(int64 d) public returns(int64)
	{
			...
			{
					d += 0;
					return Double.longBitsToDouble(Double.doubleToRawLongBits(d) + ((d >= 0) ? +1; : -1))
			}
 ...
 */
	...
	function scalb(int32 f, int32 scaleFactor) public returns(int32)
	{
         ...
         */
			...
			return int32((int64(f) * powerOfTwoD(scaleFactor)));
	}
 ...
 */
	...
	{
			return Double.longBitsToDouble(((int64(n) + int64(DoubleConsts.EXP_BIAS)) << (DoubleConsts.SIGNIFICAND_WIDTH - 1)) & DoubleConsts.EXP_BIT_MASK);
	}
