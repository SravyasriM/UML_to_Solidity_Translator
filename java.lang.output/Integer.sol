 ...
 */
	...
	Class static  immutable TYPE = Class(Class(Class.getPrimitiveClass("int")));
 ...
 */
	...
	function toUnsignedString0(int32 val, int32 shift) private returns(String)
	{
			...
			int32 mag = int32(Integer.SIZE - Integer.numberOfLeadingZeros(val));
			int32 chars = int32(Math.max(((mag + (shift - 1)) / shift), 1));
 ...
 */
	...
	function toString(int32 i) public returns(String)
	{
			...
			{
				return "-2147483648";			}
			int32 size = int32((i < 0) ? stringSize(-i;) + 1 : stringSize(i)
			getChars(i, size, buf);
			return 
	}
 ...
 */
	...
	{
			for(int32 i = int32(0); ; i++;)
			{
				if(x <= sizeTable[uint256(i)])
				{
					return i + 1;				}			}
	}
 ...
 */
	...
	function parseInt(string s, int32 radix) public returns(int32)
	{
			...
			if(len > 0)
			{
					byte firstChar = byte(s.charAt(0));
					if(firstChar < "0")
					{
							i++;
					}
...
len <= 5 || (radix == 10 && len <= 9))
							...
							{
									int64 ell = int64(Long.parseLong(s, radix));
									if((ell & 0xffff_ffff_0000_0000) == 0)
									{
											return int32(ell);
									}
									else
 ...
 */
	...
	function valueOf(int32 i) public returns(Integer)
	{
			...
			{
				return IntegerCache.cache[uint256(i + (-IntegerCache.low;))]			}
			return 
	}
 ...
 */
	...
	{
			Integer result = Integer(getInteger(nm, none));
			return (result == none) ? Integer.valueOf(val) : result;
	}
 ...
 */
	...
	function decode(string nm) public returns(Integer)
	{
			...
			bool negative = bool(false);
			Integer result;
			byte firstChar = byte(nm.charAt(0));
 ...
 */
	...
	{
			return compare(IntegerCache.value, anotherInteger.value);
	}
 ...
 */
	...
	function divideUnsigned(int32 dividend, int32 divisor) public returns(int32)
	{
			...
			return int32((toUnsignedLong(dividend) / toUnsignedLong(divisor)));
	}
 ...
 */
	...
	int64 static  immutable serialVersionUID = int64(1360826667806852920);
 ...
 */
	...
	{
			IntegerCache.value = parseInt(s, 10);
	}
 ...
 */
	...
	function getChars(int32 i, int32 index, byte[] memory buf) internal
	{
			...
			for( ; )
			{
					...
					r = i - ((q << 3) + (q << 1));
					buf[uint256(--charPos;)] = digits[uint256(r)]
					i = q;
					if(i == 0)
					{
						break;					}
